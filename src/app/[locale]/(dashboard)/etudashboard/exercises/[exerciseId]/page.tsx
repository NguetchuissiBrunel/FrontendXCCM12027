...
export default function ExerciseDetailsPage() {
        ...
        return (
            <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white dark:from-gray-900 dark:to-gray-800 flex items-center justify-center p-4">
                <div className="text-center max-w-md">
                    ...
                    <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">{t('notFound.title')}</h2>
    ...
    const displayName = `${user?.firstName || ''} ${user?.lastName || ''}`.trim() || user?.email || t('studentDefault');
    const userRole = user?.specialization || t('studentDefault');
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-3xl p-6 md:p-10 shadow-xl shadow-purple-500/5 border border-purple-100 dark:border-gray-700">
                        <div className="flex flex-col md:flex-row justify-between items-start gap-6 mb-8">
                            <div className="flex-1">
                                <span className="px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 mb-4 inline-block">
                                    {exercise.courseTitle || t('exerciseLabel')}
                                </span>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
                            <div className="flex items-center gap-4 p-4 rounded-2xl bg-gray-50 dark:bg-gray-700/50 border border-gray-100 dark:border-gray-700">
                                ...
                                <div>
                                    <div className="text-xs text-gray-500 dark:text-gray-400 font-medium">{t('dueDate')}</div>
                                    <div className="font-bold text-gray-900 dark:text-white">
                                        {exercise.dueDate ? new Date(exercise.dueDate).toLocaleDateString('fr-FR') : t('noDueDate')}
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center gap-4 p-4 rounded-2xl bg-gray-50 dark:bg-gray-700/50 border border-gray-100 dark:border-gray-700">
                                ...
                                <div>
                                    <div className="text-xs text-gray-500 dark:text-gray-400 font-medium">{t('questions')}</div>
                                    <div className="font-bold text-gray-900 dark:text-white">
                                        {exercise.questions?.length || 0} {t('items')}
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center gap-4 p-4 rounded-2xl bg-gray-50 dark:bg-gray-700/50 border border-gray-100 dark:border-gray-700">
                                ...
                                <div>
                                    <div className="text-xs text-gray-500 dark:text-gray-400 font-medium">{t('attempts')}</div>
                                    <div className="font-bold text-gray-900 dark:text-white">{t('singleAttempt')}</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col sm:flex-row items-center gap-4">
                            {canSubmit ? (
                                ...
                                <div className="w-full p-4 rounded-2xl bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-900/30 flex items-center gap-3 text-red-600 dark:text-red-400">
                                    <AlertCircle size={20} />
                                    <span className="font-medium">{reason || t('cannotSubmit')}</span>
                                </div>
                            )}
